...
class LiveScreenService : Service() {
    ...
    private fun fetchEphemeralTokenAndConnect() {
        val request = Request.Builder()
            .url("$backendUrl/live-token")
            .post(ByteArray(0).toRequestBody("application/json".toMediaType()))
            .build()

        httpClient.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                showAnswer("Live token error: ${e.message ?: "network error"}")
            }

            override fun onResponse(call: Call, response: Response) {
                response.use {
                    if (!it.isSuccessful) {
                        showAnswer("Live token error: HTTP ${it.code}")
                        return
                    }
                    ...
                    try {
                        ...
                        val model = json.optString("model", "gemini-3.1-flash-live-preview")
                        connectGemini(token, model)
                    } catch (e: Exception) {
                        showAnswer("Live token response invalid hai.")
                    }
                }
            }
        })
    ...
    private fun connectGemini(token: String, model: String) {
        ...
        webSocket = httpClient.newWebSocket(request, object : WebSocketListener() {
            override fun onOpen(webSocket: WebSocket, response: Response) {
                    ...
                    .put("setup", JSONObject()
                        ...
                        .put("systemInstruction", JSONObject()
                            .put("parts", org.json.JSONArray().put(
                                JSONObject().put(
                                    "text",
                                    "Tum Annotate Agent ho. User ke phone screen ke live frames ko dekho. " +
                                        "User Hindi/Hinglish me sawaal kare to concise Hindi/Hinglish me jawab do. " +
                                        "Visible buttons, errors, text aur UI ko explain karo. " +
                ...
                showAnswer("🟢 Live connected. Ab bubble tap karke sawaal poochho.")
            }
    ...
    private fun parseGeminiMessage(raw: String) {
        try {
            ...
            // Ignore non-text Live events such as setup/usage metadata.
        }
    }
    ...
    private fun handleImage(image: Image) {
        try {
            ...
            val scaled = if (cropped.width > MAX_FRAME_WIDTH) {
                val newHeight = cropped.height * MAX_FRAME_WIDTH / cropped.width
                Bitmap.createScaledBitmap(cropped, MAX_FRAME_WIDTH, newHeight, true)
            } else {
                cropped
            }
            ...
            // A frame can be discarded if Android rotates or tears down the projection.
        } finally {
            image.close()
        }
    }

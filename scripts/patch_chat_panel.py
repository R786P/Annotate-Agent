...
# Replace the native panel with a Render-hosted WebView. The native bridge keeps
# clipboard, upload, voice and sending local, while the visual UI can change on
# Render without rebuilding the APK.
...
panel_new = '''    private fun buildPanel() {
        ...
        val web = WebView(this).apply {
            ...
            loadUrl("$backendUrl/mobile-panel")
        }
        chatWebView = web
    ...
    private inner class PandaWebBridge {
        ...
        fun send(text: String) {
            val q = text.trim()
            if (q.isNotBlank()) sendText(q)
        }

        @JavascriptInterface
        fun voice() { startVoiceInput() }
        ...
        fun openFilePicker() {
            try {
                ...
                pushReply("📎 File picker open nahi hua.")
            }
        }
    }
...
# Route all replies into the hosted panel.
...
if count != 1:
    raise SystemExit("showAnswer block not found")
